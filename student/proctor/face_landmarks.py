...
def detect_landmarks(frame, faces, det_conf=0.7, track_conf=0.7):  # Assuming only one face in each face object
    ...
    with mp_face_mesh.FaceMesh(static_image_mode=False, max_num_faces=2, refine_landmarks=True,
                               min_detection_confidence=det_conf, min_tracking_confidence=track_conf) as face_mesh:
        ...
        for face in faces:
            ...
            if results.multi_face_landmarks:
                ...
                face.hland = np.array([(((lm.x * w) + x) / f_w, ((lm.y * h) + y) / f_h, lm.z) for lm in
